...
build: ## Build package - removed unittest
	mvn -U -ntp -s .github/maven.settings.xml -e -DskipTests=true -Dimage.tag=$(cat image.tag) clean install de.qaware.maven:go-offline-maven-plugin:resolve-dependencies -Pgwt-production,unit-tests,docker  -Dbamboo.buildNumber=0
